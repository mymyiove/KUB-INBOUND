document.addEventListener("DOMContentLoaded", () => {
    ...
    function autoFormatPhone(e) {
        ...
        if (length < 4) { e.target.value = value; } 
        else if (length < 7) { e.target.value = `${value.substring(0,3)}-${value.substring(3)}`; } 
        else if (length < 11) { e.target.value = `${value.substring(0,3)}-${value.substring(3,6)}-${value.substring(6)}`; } 
        else { e.target.value = `${value.substring(0,3)}-${value.substring(3,7)}-${value.substring(7)}`; }
    }
